...
internal class Program
{
	...
	Why we create a list of manifacturers:
		...
		I'm sure there is an API containing all this information in the real world - but this is just a plaything.
	*/
	...
	Explanation GetManifacturerName:
		...
		If the card matches 2 or more manifacturers, return UNKNOWN. (Should never happen)
	...
	*/
	...
	{
		// Cache a null value that will be replaced if a manifacturer is found.
	...
	*/
	...
	static bool CheckSumValidation(string cardNumber)
	{
		...
		// Parse all numbers individually and simultaneously check for invalid characters.
		...
		// This is the early return pattern.
		if (validationNumber[validationNumber.Length - 1] == '0')
			return true;

		// Since we didn't return the card doesn't end in 0 and is not valid.
		return false;
	}
	...
	// Edit: 
	// I heard the compiler makes this optimisation for all modulus 2 (%2) operands automatically. I have not confirmed.
	// I still believe it to be better this way though as in a modulus method we might want to have a property for the modulus operand.
	// Making it impossible for the compiler to optimize as it'd use runtime values.
